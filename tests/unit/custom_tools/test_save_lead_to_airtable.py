...
@patch(
    "nalgonda.custom_tools.save_lead_to_airtable.EnvConfigManager.get_by_key",
...
def test_save_lead_to_airtable_success(mock_table, mock_get_by_key):
    ...
    tool = SaveLeadToAirtable(name="Jane Doe", email="jane@test.com", lead_details="Interested in service Y")
    response = tool.run()
...
def test_save_lead_to_airtable_failure(mock_table, mock_get_by_key):
    ...
    tool = SaveLeadToAirtable(name="John Smith", email="johnsmith@test.com", lead_details="Looking for consulting.")
    response = tool.run()
