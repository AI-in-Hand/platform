...
def test_empty_agency_chart():
    ...
    config = AgencyConfig(agency_id="123", agents=["Agent1", "Agent2"], agency_chart=[])
...
def test_main_agent_not_in_agency_chart():
    ...
    with pytest.raises(ValueError) as excinfo:
        AgencyConfig(
            agency_id="123", agents=["Agent1", "Agent2"], main_agent="CEO", agency_chart=[["Agent1", "Agent2"]]
        )
...
def test_duplicate_agents_in_agency_chart():
    ...
    with pytest.raises(ValueError) as excinfo:
        AgencyConfig(agency_id="123", agents=["Agent1", "Agent2"], main_agent="CEO", agency_chart=[["CEO", "CEO"]])
    assert "List must be unique" in str(excinfo.value)


def test_main_agent_not_set():
    with pytest.raises(ValueError) as excinfo:
        AgencyConfig(agency_id="123", agents=["Agent1", "Agent2"], agency_chart=[["Agent1", "Agent2"]])
