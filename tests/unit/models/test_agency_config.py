...
def test_empty_agency_chart():
    ...
    config = AgencyConfig(agency_id="123", name="Test agency", agents=["Agent1", "Agent2"], agency_chart=[])
...
def test_main_agent_not_set():
    with pytest.raises(ValueError) as excinfo:
        AgencyConfig(
            agency_id="123", name="Test agency", agents=["Agent1", "Agent2"], agency_chart=[["Agent1", "Agent2"]]
        )
