...
class UserSecretStorage:
    ...
    def get_all_secrets(self, user_id: str) -> dict | None:
        """Fetch user secrets based on user_id"""
    ...
    def set_secrets(self, user_id: str, secrets: dict[str, str]) -> None:
        """Set user secrets based on user_id"""
        logger.info(f"Setting user secrets for user_id: {user_id}")
        collection = self.db.collection(self.collection_name)
        collection.document(user_id).set(secrets)

    def update_secrets(self, user_id: str, secrets: dict[str, str]) -> None:
        logger.info(f"Updating user secrets for user_id: {user_id}")
        collection = self.db.collection(self.collection_name)
        collection.document(user_id).update(secrets)
